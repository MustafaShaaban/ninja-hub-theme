<?php
    /**
     * @Filename: class-nh_file_uploader.php
     * @Description:
     ...
     * @requirenments [Nh_Cryptor, Nh_Forms, Recaptcha, JS Files]
     *
    ...
    class Nh_File_Uploader
    {
        ...
        protected function actions(): void
        {
            ...
            $this->hooks->add_action('wp_ajax_' . Nh::_DOMAIN_NAME . '_upload_attachment', $this, 'upload');
            $this->hooks->add_action('wp_ajax_' . Nh::_DOMAIN_NAME . '_remove_attachment', $this, 'remove');
        }
        ...
        public function upload(): void
        {
            ...
            if (!empty($file)) {
                ...
                if (($_FILES['file']['size'] >= self::MAX_SIZE_UPLOAD) || ($_FILES["file"]["size"] == 0)) {
                    new Nh_Ajax_Response(FALSE, __("File too large. File must be less than 5 megabytes.", 'ninja'));
                }

                $acceptable = apply_filters('nh_file_uploader_accepted_extensions', [
                    'image/jpeg',
                    'image/jpg',
                    'image/png'
                ]);
